...
class HospitalExpenses(Document):
    def on_submit(self):
        ...
        if existing_receipt:
            frappe.msgprint(_(f"Hospital Expenses already exists for {self.name}"))
            return
        ...
        journal_entry.company = 'Royal Clouds Medical Center'
        ...
        frappe.db.commit()
        frappe.msgprint(_(f"Successfully Posted  Hospital Expenses for {self.name}"))
