...
@frappe.whitelist()
def create_vital_signs_for_patient(doc_name=None,doc=None,method=None):
...
@frappe.whitelist()
def create_vital_signs_patient(doc=None,method=None):
    # Check if a draft Vital Signs document already exists for this patient
    existing_vital_signs = frappe.get_all("Vital Signs", filters={
        "patient": doc.name,
        "custom_patient_status": "Seen The Receptionist",
    ...
    if not existing_vital_signs:
        ...
        try:
            vital_signs = frappe.get_doc({
                "doctype": "Vital Signs",
                "patient": doc.name,
                "custom_practionaer": frappe.get_doc("Patient", doc.name).custom_consulting_doctor,
                "custom_patient_status": "Seen The Receptionist",
                "custom_customer_type": frappe.get_doc("Patient", doc.name).customer_group,
                "custom_invoice_no": frappe.get_doc("Patient", doc.name).custom_invoice_no,
            })
            vital_signs.insert(ignore_permissions=True)
            return "Vital Signs created successfully."
        except Exception as e:
